...
function DisplayFiles($par_category_id) {
	...
	//echo $sql."<br>";
	$result = $db->Query($sql);
	$s="<table class='teamspace' width='500' cellpadding='2' cellspacing='0'>\n";
				...
				*/
				...
				if ($row['is_current'] == "y" || ($row['is_pending'] == "y" && $row['user_id'] == $user_id)) {
					...
					if ($row['checked_out'] == "y") {
						...
						$checked_out_user=GetColumnValue("full_name","user_master","user_id",$row['user_id_checked_out'],"");
					...
					if ($row['locked'] == "y") {
						...
						$locked_user=GetColumnValue("full_name","user_master","user_id",$row['user_id_locked'],"");
