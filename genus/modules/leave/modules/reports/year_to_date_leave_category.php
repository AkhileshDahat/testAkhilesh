...
function SubTask() {
	...
	/* GRAB THE DATA HERE */
	$obj_gd=new GenData;
	$sql="SELECT count(*) as total, cm.category_name as description
				...
				";
	$obj_gd->GenArrayData($sql);
	$obj_gd->GenPiePercentages();
	$arr_data_values=$obj_gd->GetVar("arr_totals");

	$arr_data_index=$obj_gd->GetVar("arr_desc");

	/* CONTINUE WITH THE GEN XML STUFF */
	$obj_gxf->SetVar("header","Current Year Leave Categories");
	/* SET THE ARRAY OF DATA FROM ABOVE */
	$obj_gxf->SetVar("arr_data_values",$arr_data_values);
	$obj_gxf->SetVar("arr_data_index",$arr_data_index);
	...
	$c.="<div id=\"chart\" class=\"chart\">\n";
		$c.="<embed type=\"application/x-shockwave-flash\" src=\"bin/reporting/swf/fcp-pie-chart.swf\" id=\"flashchart\" name=\"flashchart\" bgcolor=\"#ffffff\" quality=\"high\" flashvars=\"config=bin/reporting/xml/pie3d_".$_SESSION['sid'].".xml\" height=\"500\" width=\"700\">\n";
	$c.="</div>\n";
	$c.="<script type=\"text/javascript\">\n";
	$c.="var source = new SWFObject('bin/reporting/swf/fcp-pie-chart.swf', 'flashchart', '700', '500', 5, \"#ffffff\");\n";
	$c.="source.addVariable(\"config\", 'bin/reporting/xml/pie3d_".$_SESSION['sid'].".xml');source.write(\"chart\");\n";
	$c.="</script>\n";
