...
function BrowseWorkspaceModuleRoles($workspace_id,$module_id) {
	...
	$sr->Query("SELECT cwrm.role_id,cwrm.role_name,CASE WHEN csma.module_id IS NULL THEN 'No' ELSE 'Yes' END AS installed
	...
	for ($i=0;$i<$sr->CountRows();$i++) {
		...
		$sr->ModifyData($i,2,"<a href='index.php?module=workspace&task=workspace_module_roles&subtask=grant_access&workspace_id=".$workspace_id."&module_id=".$module_id."&role_id=".$role_id."&installed=".STRTOLOWER($installed)."' title='Click to change access'>".$installed."</a>");
	}
	$sr->RemoveColumn(0);
	//$sr->RemoveColumn(1);
	...
	$sr->FooterLinks("index.php?module=workspace&task=installed_workspace_modules&workspace_id=".$workspace_id,"Back");
