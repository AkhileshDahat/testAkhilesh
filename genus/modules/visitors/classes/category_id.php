...
class CategoryID {
	...
	private function Info() {
		...
		$sql="SELECT category_name
					FROM ".$GLOBALS['database_prefix']."visitor_category_master
	...
	public function Add() {
		...
		if (RowExists("visitor_category_master","category_name","'".$this->category_name."'","AND workspace_id=".$GLOBALS['workspace_id']." AND teamspace_id ".$GLOBALS['teamspace_sql'])) { $this->Errors("department name exists. Please choose another."); return False; }

		/* DATABASE CONNECTION */
		$db=$GLOBALS['db'];

		$sql="INSERT INTO ".$GLOBALS['database_prefix']."visitor_category_master
	...
	public function Edit($category_name) {
		...
		if (RowExists("visitor_category_master","category_name","'".$category_name."'","AND workspace_id=".$GLOBALS['workspace_id'])) { $this->Errors("department name exists. Please choose another."); return False; }

		/* DATABASE CONNECTION */
		$db=$GLOBALS['db'];

		$sql="UPDATE ".$GLOBALS['database_prefix']."visitor_category_master
