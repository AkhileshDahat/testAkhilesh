...
function BrowseReserveTimetable() {
	...
	$c.="<br />\n";
	$c.="<input type='button' name='reser_button' value='Reset All' onClick=\"document.location.href='index.php?module=scheduling&task=reserve_slots&reset=yes'\">";
...
function DrawButton($p_day,$p_start_time,$p_end_time) {
	global $obj_slrs;
	if ($obj_slrs->SlotIsReserved($_SESSION['user_id'],$p_start_time,$p_end_time,$p_day,"own")) {
		return "Booked";
	}
	else {
		return "<input type='button' name='block_button' value='Block' onClick=\"document.location.href='index.php?module=scheduling&task=reserve_slots&save=yes&time_start=$p_start_time&time_end=$p_end_time&day_of_week=$p_day'\">\n";
	}
}
