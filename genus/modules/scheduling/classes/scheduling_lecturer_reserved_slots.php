...
class SchedulingLecturerReservedSlots {
	...
	public function SlotIsReserved($p_user_id,$p_time_start,$p_time_end,$p_day_of_week,$reserve_type) {
		if (RowExists("scheduling_lecturer_reserved_slots","user_id",$p_user_id,"AND time_start='".$p_time_start."' AND time_end = '".$p_time_end."' AND day_of_week = '".$p_day_of_week."' AND reserve_type = '".$reserve_type."' AND workspace_id = ".$GLOBALS['workspace_id']." AND teamspace_id ".$GLOBALS['teamspace_sql'])) {
			return True;
		}
		else {
			return False;
