...
function AddWorkspace() {
	...
	*/
	if (ISSET($_POST['add_workspace'])) {
		...
		*/
		if (!$_GET['workspace_id']) {
			...
			if ($result) {
				...
				$GLOBALS['errors']->SetAlert("Success, workspace added successfully");
				$show_submit=False;
			}
			else {
				$GLOBALS['errors']->SetError($p->ShowErrors());
			}
		}
		/*
			EDITING WORKSPACE
		*/
			...
			if ($result) {
				$GLOBALS['errors']->SetAlert("Success, workspace edited successfully");
				$show_form=False;
			}
			else {
				$GLOBALS['errors']->SetError($p->ShowErrors());
		...
		require_once $GLOBALS['dr']."modules/core/classes/workspace_id.php";

		$workspace_id=$_GET['workspace_id']; // WE NEED TO INSTANTIATE THE VARIABLE

		$wid=new WorkspaceID();
		$result = $wid->SetParameters($workspace_id);
		if (!$result) { return $wid->ShowErrors(); }
	...
	*/
	...
	$c="<table class='plain_border' cellspacing='0' cellpadding='5' width='500'>\n";
		...
		$c.="</tr>\n";
		$c.="<form method='post' name='add_workspace' action='index.php?module=core&task=add_workspace&workspace_id=".$workspace_id."' onsubmit='return ValidateForm(this);'>\n";
		...
		$c.="</tr>\n";
		*/
		$c.="<input type='hidden' name='max_teamspaces' value='0'>\n";
		$c.="<input type='hidden' name='max_size' value='0'>\n";
		$c.="<input type='hidden' name='max_users' value='0'>\n";
		$c.="<tr>\n";
		...
		$c.="</tr>\n";
		*/
		$c.="<input type='hidden' name='enterprise' value='0'>\n";
		$c.="<tr>\n";
			$c.="<td>Status</td>\n";
