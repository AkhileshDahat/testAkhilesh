...
require_once $GLOBALS['dr']."modules/core/classes/signup.php";
...
function SignupForm() {
	...
	if (ISSET($_POST['full_name'])) {
		...
		if (ISSET($_POST['community_join_add']) && $_POST['community_join_add'] == "add") {
			...
			//$access_code = $obj_signup->GetInfo("access_code");
			//include $GLOBALS['dr']."include/templates/signup.php";
		}
		else {
		...
		if (!$result_signup) {
			...
			$db->Rollback();
			//echo $obj_signup->ShowErrors();
			$GLOBALS['errors']->SetError($obj_signup->ShowErrors());
			...
			$GLOBALS['errors']->SetAlert("Signup successful. Please see your mail to activate your account");
			//$db->Rollback();// testing
			$db->Commit();
			$title_id="";
			$full_name="";
	...
	*/
	...
	$c.="<table class='plain'>\n";
		...
		$c.="</tr>\n";
		$c.="<form method='post' enctype='multipart/form-data' name='signup_form' action='index.php?module=core&task=signup' onsubmit='return ValidateForm(this);'>\n";
		$c.="<tr>\n";
		...
		*/
		...
		$c.="<tr>\n";
			...
			$c.="<td colspan='2'><nobr>".DrawInput("workspace_code","input_reqd",$workspace_code,"10","100","onKeyUp=\"frames['workspace_code_lookup'].location.href='modules/core/bin/workspace_code_lookup.php?workspace_code='+document.getElementById('workspace_code').value\" autocomplete=off")."<iframe id='workspace_code_lookup' name='workspace_code_lookup' src='modules/core/bin/workspace_code_lookup.php' width='200' height='25'scrolling='no' border='0' style='border:none;' frameborder='0'></iframe></nobr></td>\n";
		$c.="</tr>\n";
